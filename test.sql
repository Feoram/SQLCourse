...
SELECT s.name                       AS skill,
       COUNT(DISTINCT r.vacancy_id) AS vacancy_count
FROM skills s
         JOIN requirements r ON s.id = r.skill_id
GROUP BY s.id
...
SELECT v.title, v.location, v.salary
FROM vacancies v
         JOIN (SELECT location, AVG(salary) AS avg_salary
               FROM vacancies
               GROUP BY location) avg_table ON v.location = avg_table.location
WHERE v.salary > avg_table.avg_salary;

-- 13. Вакансии с самыми длинными требованиями (по количеству нужных навыков).
SELECT v.id,
       v.title,
       COUNT(r.skill_id) AS skills_required
FROM vacancies v
...
SELECT e.name,
       AVG(v.closed_date - v.published_date) AS avg_days_to_close
FROM vacancies v
...
FROM (SELECT v.id,
      ...
      GROUP BY v.id, v.location) AS skill_stats
GROUP BY location
...
SELECT s.name                  AS skill,
       ROUND(AVG(v.salary), 2) AS avg_salary
FROM skills s
         JOIN requirements r ON s.id = r.skill_id
...
SELECT e.name                                                                     AS employer,
       ROUND(COUNT(*) / COUNT(DISTINCT DATE_TRUNC('month', v.published_date)), 2) AS avg_vacancies_per_month
FROM vacancies v
         JOIN employers e ON v.employer_id = e.id
GROUP BY e.name
ORDER BY avg_vacancies_per_month DESC
LIMIT 5;


-- 19. 5 городов с самой высокой конкуренцией (самое большое число вакансий на одного работодателя).
SELECT location,
       ROUND(COUNT(*)::NUMERIC / COUNT(DISTINCT employer_id), 2) AS vacancies_per_employer
FROM vacancies
GROUP BY location
ORDER BY vacancies_per_employer DESC
LIMIT 5;


-- 20. В каких компаниях работают специалисты с самым большим количеством навыков?
SELECT e.name                     AS employer,
       ROUND(AVG(skill_count), 2) AS avg_skills_per_vacancy
FROM (SELECT v.id, v.employer_id, COUNT(r.skill_id) AS skill_count
      FROM vacancies v
               LEFT JOIN requirements r ON v.id = r.vacancy_id
      GROUP BY v.id, v.employer_id) AS vacancy_skills
         JOIN employers e ON vacancy_skills.employer_id = e.id
GROUP BY e.name
ORDER BY avg_skills_per_vacancy DESC
LIMIT 5;
